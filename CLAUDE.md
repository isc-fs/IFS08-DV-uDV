...
- **FDCAN2**: ACU bus (ECU/VCU + AMS) — the ECU↔uDV contract, matching `IFS08-CE-ECU`'s `.def` files. Receives `0x504` TS-active, `0x505` brake-over-limit verdict, `0x506` motor rpm, `0x511` DV R2D confirm. Transmits `0x507` torque (int32 percent, 20 ms) and `0x510` DV R2D request. RX FIFO0 is routed to interrupt LINE1 (only `FDCAN2_IT1` is NVIC-enabled).
- **FDCAN3**: AMI + steering bus (local DV peripherals). Receives mission select `0x503`, steering sensor `0x2B0`, steering feedback `0x500`. Transmits mission ACK `0x50A`, ASSI `0x100` (local bus only — the ECU→AMS heartbeat also uses 0x100 but on the ACU bus), steering motor `0x010` and angle `0x020`.
