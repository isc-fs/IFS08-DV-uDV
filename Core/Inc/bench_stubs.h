 ...
 * while there is no EBS-pneumatics rig / DVPC on the bench. REVERT ALL TO 0
 * before any production / competition run (the firmware still announces
 * "BENCH STUBS COMPILED IN" on /debug at boot so a stubbed image is
 * obvious). */
