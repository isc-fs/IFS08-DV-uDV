...
- Includes our custom ROS interface package(s) compiled into
  libmicroros (the firmware currently uses only stock packages —
  `std_srvs`, `std_msgs`, `sensor_msgs`, `geometry_msgs` — so a stock
  rebuild also works — see `Core/Src/ros_task.c`).
