...
- **`libmicroros.a` rebuild (`MAX_SERVICES`).** The firmware creates **two**
  service servers (`activate_steering`, `force_ebs`). `libmicroros.a` is not
  ...
  **rebuild the static library** (see CLAUDE.md → "Rebuild micro-ROS static
  library") and reflash so the linked lib matches. The three subscriptions
  (`cmd_test`, `dv/status`, `ctrl/cmd`) already fit the executor's 5 handles
  and the subscription limit, so they need no rebuild.
...
1. **5 s Ready dwell vs. the Driving transition.** The READY→DRIVING
   ...
   ([app_task.cpp:568](../Core/Src/app_task.cpp)) but is now **write-only** —
   nothing reads it, so the 5 s dwell currently gates nothing (the old
   `0x509` R2D frame it once guarded is gone; the DV R2D request is now the
   ungated `0x510` `sendR2dRequest`). If the rules require ≥5 s in AS Ready
   before Driving and the RES does not enforce it itself, add
